...
public interface HourlyWsStationDAO extends JpaRepository<HourlyWsStation,Integer> {
    ...
    /** Өмнөх ээлжийн (D-1 өдрийн 08:00 – D өдрийн 07:00) сүүлийн бүртгэл станц бүрт — FM baseline */
    @Query(value = """
    SELECT h.*
    FROM hourly_ws_station h
    INNER JOIN (
        ...
        WHERE menu_id IN :menuIds
          AND (
                (DATE(date) = :prevDateStr AND hour >= 8)
             OR (DATE(date) = :dateStr     AND hour <= 7)
          )
        GROUP BY menu_id
    ) latest ON h.id = latest.max_id
    """, nativeQuery = true)
    ...
    /** Тухайн станцын өмнөх ээлжийн (D-1 өдрийн 08:00 – D өдрийн 07:00) хамгийн сүүлийн бүртгэл */
    @Query(value = """
